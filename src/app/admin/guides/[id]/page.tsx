...
import EditGuideHeaderForm from "./EditGuideHeaderForm";
...
export default async function GuideDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const dateStr = guide.date.toISOString().split("T")[0];
    ...
    <div>
      <div className={styles.pageHeader}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          <DeleteGuideButton guideId={guide.id} sunatNumber={guide.sunatNumber} />
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "2rem", marginTop: "2rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          {/* Edit form below the info card */}
          <EditGuideHeaderForm
            guideId={guide.id}
            sunatNumber={guide.sunatNumber}
            date={dateStr}
            notes={guide.notes || ""}
          />
        </div>
              ...
              <div key={det.id} style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "1rem", border: "1px solid var(--card-border)", borderRadius: "var(--radius)" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "0.875rem", color: "var(--text-muted)" }}>
                    ...
                    {det.size && <span> — Talla: <strong>{det.size}</strong></span>}
                  </div>
                </div>
