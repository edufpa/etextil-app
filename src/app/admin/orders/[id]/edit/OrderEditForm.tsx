...
export default function OrderEditForm({ order, allServices, allSizes, allColors }: OrderEditFormProps) {
  ...
  const [services, setServices] = useState<{ service_id: number; requiredQuantity: number; notes: string }[]>(
    order.services.map((s: any) => ({ service_id: s.service_id, requiredQuantity: s.requiredQuantity, notes: s.notes || "" }))
  );
