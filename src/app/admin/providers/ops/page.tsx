...
import { ArrowLeft, Send, PackageCheck, ClipboardList } from "lucide-react";
...
type SearchParams = Promise<{ service?: string; provider?: string }>;
...
export default async function OpenOpsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const totalPending = filtered.reduce((s, d) => {
    const inQty = d.incomings.reduce((a, i) => a + i.quantity, 0);
    return s + (d.quantity - inQty);
  }, 0);
    ...
    <div>
      <div className={styles.pageHeader}>
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          ...
          <h1 className={styles.title} style={{ marginBottom: 0 }}>
            ...
            OPs Abiertas — Servicios en Proceso
          </h1>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", margin: "1.5rem 0" }}>
        <div style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: "var(--radius)", padding: "1.25rem" }}>
          ...
          <div style={{ fontSize: "2rem", fontWeight: 800, color: "var(--primary)", marginTop: "0.25rem" }}>{totalOps}</div>
        </div>
      ...
      {byService.size === 0 ? (
        <div style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: "var(--radius)", padding: "3rem", textAlign: "center", color: "var(--text-muted)" }}>
          No hay OPs abiertas con los filtros seleccionados. ¡Todo al día!
        </div>
      ) : (
              ...
              <div key={serviceName} style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: "var(--radius)", overflow: "hidden" }}>
                ...
                <div style={{ overflowX: "auto" }}>
                  <table className={styles.table} style={{ marginBottom: 0 }}>
                    ...
                    <tbody>
                      {ops.map((op: any) => (
                        <tr key={op.id} style={{ background: "#fff8f0" }}>
                          <td>
                          ...
                          <td style={{ textAlign: "center", fontWeight: 700, color: "orange", fontSize: "1rem" }}>
                            {op.pending}
                          </td>
                          <td>
                            <Link href={`/admin/orders/${op.orderId}`} style={{ fontSize: "0.78rem", color: "var(--primary)" }}>
                              Ver pedido →
                            </Link>
                          </td>
                        </tr>
