...
export default function ReporteTalleresFilters({ services, providers }: Props) {
  ...
  const clear = () => router.push("/admin/providers/reporte");
  ...
  return (
    <div style={filterBarStyle}>
      ...
      <div style={fieldStyle}>
        <label style={labelStyle}>Solo activos</label>
        <select value={searchParams.get("pending") || ""} onChange={(e) => update("pending", e.target.value)}>
          <option value="">Todos</option>
          <option value="1">Con trabajo pendiente</option>
        </select>
      </div>
      {hasFilters && (
